...
static const flexcan_user_config_t *g_config[CAN1_INDEX + 1] =
{
#if defined INST_CANCOM0
	&canCom0_InitConfig0,
#else
...
static mutex_t           g_tx_mutex[CAN1_INDEX + 1];                      /**< Tx mutex */
...
uint8_t can_transmit(const uint8_t _index, const uint32_t _id, const uint8_t *const _buf, const uint8_t _size)
{
	...
	/* Configure TX message buffer with index message_id and g_tx_mailbox[_inst] */
